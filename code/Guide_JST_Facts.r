...
p$ma5.capital_real = ((1+p$lag4.capital_real)*(1+p$lag3.capital_real)*(1+p$lag2.capital_real)*(1+p$lag1.capital_real)*(1+p$capital_real))^(1/5) - 1
...
fig <- layout(fig, title = list(text = 'Return to capital over time', x=0),
              ...
              yaxis = list (title = '5-year average return',range = c(-.1,.2)),
              hovermode="x unified")

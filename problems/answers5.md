...
As in the Solow model, it is easier to see the consequences of these changes by simulating the model. To see this in figures that the problem asks for, it is helpful to use [Solow model app](https://dietzvollrath.shinyapps.io/ProdBasic/). The app is a simulation of the productivity dynamics in the Romer model, and allows you to adjust the parameters to see what happens in response. 
